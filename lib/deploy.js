...
function replaceAppInstances(domain, hosts, image, fn) {

}

function getAvailableNodes(fn) {
  countAllRunningContainers(function(err, counts) {
    if (err) {
      return fn(err);
    }
    var nodes = _(counts).sortBy(function(t) {
      return t[1];
    }).last(2).pluck(0).value();
    fn(null, nodes);
  });
}

function deployNewAppInstances(domain, image, fn) {
  getAvailableNodes(function(err, nodes) {
    async.map(nodes, function(node, fn) {
...
function deployAppInstances(domain, image, fn) {
  service.loadAppHosts(domain, function(err, hosts) {
    ...
    if (hosts) {
      replaceAppInstances(domain, hosts, image, fn);
    }
    else {
